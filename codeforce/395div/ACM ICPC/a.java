    ...
    try{
     ...
     // t=ii();
      ...
      for (int i=0;i<str.length() ;i++ ) {
         if(al[str.charAt(i)-97].size()==0)
          al[str.charAt(i)-97].add(-1);
         al[str.charAt(i)-97].add(i);
      }
      for (int i=0;i<n;i++ ) {
        al[str.charAt(i)-97].add(n);
      }
      int ans=1;
      if(n % 2==0)
         ans=n/2+1;
      ...
      for (int i=0;i<26 ;i++ ) {
         int max=Integer.MIN_VALUE;
          ...
          for (int j=1;j<al[i].size() ;j++ ) {
               ...
               int diff=Math.abs((int)al[i].get(j) - (int)al[i].get(j-1));
               if(diff>max)
                 max=diff;
              //System.out.println("entered "+diff);
          }
