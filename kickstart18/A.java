  ...
  void main1()
  {
    try{
      ...
      while(t-->0){
        ...
        while(temp>0){
           long d=temp%10;
           num+=d*(long)Math.pow(10,pl-1);

           if(d % 2==1){
            long incr=(d+1)*(long)Math.pow(10,pl-1)-num;
            long nn=0;
             for(long i=1;i<pl;i++){
              nn+=8L*(long)Math.pow(10,i-1);
             }
             nn+=(d-1)*(long)Math.pow(10,pl-1);
            long decr=num-nn;
            ans=Math.min(incr,decr);
            System.out.println("nn and decr"+nn+" "+incr);
           }
