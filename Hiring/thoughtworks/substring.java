...
//.............staic class end.
    ...
    try{
     ...
     // t=ii();
      ...
      long[] qq=ilong();
       ...
       for (int i=0;i<q ;i++ ) {
           long key=qq[i];
           int ind=upperBound(n,key-1);
           if(ind==0)
          out.println(inp.substring(ind,ind+(int)(key)));
          else if(ind==n)
            out.println("-1");
          else
            out.println(inp.substring(ind,ind+(int)(key-cumm[ind-1])));
  ...
  // ...............required method.
 public  int upperBound(int length, long value) {
        int low = 0;
        int high = length;
        while (low < high) {
